<?php 
      ...
      $consulta_datos = ("SELECT * FROM producto WHERE categoria_id = 1 ORDER BY producto_id ASC LIMIT $inicio,$registros");
      $consulta_total = ("SELECT COUNT(producto_id) FROM producto WHERE categoria_id = 1");
            ...
            <div class="thumbnail">
				...
				<center><a class="btn btn-success" href="accionCarrito.php?action=addToCart&id=<?php echo $row["producto_id"]; ?>">Enviar al Carrito</a></center>
                    ...
                    if($total>0 && $pagina<=$Npaginas){
                        $tabla.='<p id="texto_paginador"class="has-text-right">Mostrando productos <strong>'.$pag_inicio.'</strong> al <strong>'.$pag_final.'</strong> de un <strong>total de '.$total.'</strong></p>';
                    }
